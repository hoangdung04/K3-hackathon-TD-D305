...
const FULL_SLIDE_SELECTION: SelectionBox = { x: 0, y: 0, width: 100, height: 100 };
// A new demo session must not inherit old trial requests from an earlier build.
const SESSION_STORAGE_KEY = "vlearn-session-v2";
...
export default function Home() {
  ...
  const sessionHeaders = () => {
    let session = window.localStorage.getItem(SESSION_STORAGE_KEY);
    if (!session) {
  ...
  const submitQuestion = async () => {
    const activeContext = selection
      ? null
      : followUpContext;
    const activeSelection = selection || activeContext?.selection || FULL_SLIDE_SELECTION;
    ...
    setError("");
    try {
      const capture = selection || !activeContext
      ...
      if ((queryScope === "lesson" && !payload.analysis.needsConfirmation) || activeContext) {
        ...
        setSubmittedQuestion(null);
        setStage("draw");
        setNotice("Bạn có thể hỏi sâu hơn hoặc tiếp tục hỏi về bài học.");
        return;
      }
          ...
          <div className="conversation">
            ...
            {submittedQuestion ? (
              <div className="studentMessage">
                {selection || followUpContext?.queryScope === "region"
                  ? `Vùng đã khoanh · Câu hỏi: ${submittedQuestion}`
                  : `Câu hỏi: ${submittedQuestion}`}
              </div>
            ) : null}
            ...
            {stage === "analyzing" ? (
              <div className="assistantMessage responseCard loadingCard">
                ...
                <strong>Entropy đang nhập</strong>
              </div>
            ) : null}
